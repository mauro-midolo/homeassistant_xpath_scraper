...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    ...
    # NOTE: this is stored as a string like "kWh" or "None"
    ...
    for sensor_name in sensors_binary:
        async_add_entities([XPathScraperSensor(entry, sensor_name, coordinator)], True)


class XPathScraperEntity(CoordinatorEntity):
    def __init__(self, entry: ConfigEntry, sensor_name, coordinator: XPathScraperDataCoordinator):
        super().__init__(coordinator)
        self._entry = entry
        self._sensor_name = sensor_name

        # NEW: parse state_class from config
        state_class_str = entry.data.get(CONF_STATE_CLASS_VAR, "None")
        self._state_class = STATE_CLASS_MAP.get(state_class_str)
...
class XPathScraperSensor(XPathScraperEntity, SensorEntity):
    """Representation of an XPath Scraper sensor."""
